...
create unique index if not exists idx_oauth_sessions_authorization_code_hash on oauth_sessions(authorization_code_hash)
where authorization_code_hash is not null;
create unique index if not exists idx_oauth_sessions_access_token_hash on oauth_sessions(access_token_hash)
where access_token_hash is not null;
create unique index if not exists idx_oauth_sessions_refresh_token_hash on oauth_sessions(refresh_token_hash)
