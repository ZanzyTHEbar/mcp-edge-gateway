...
func TestReconcilerRunOncePreservesRuntimeLastError(t *testing.T) {
	...
	runtime := fakeRuntimeClient{
		resultsByTenant: map[ids.UUID]RuntimeApplyResult{
			tenantID: {
				Status:        "degraded",
...
type fakeTenantStore struct {
	...
	markedTenants      []ids.UUID
	deletedTenants     []ids.UUID
	markedTenantErrors map[ids.UUID]string
}
...
func (f *fakeTenantStore) MarkTenantReconciled(ctx context.Context, tenantID ids.UUID, reconciledAt time.Time, lastError string) error {
	if f.markedTenantErrors == nil {
		f.markedTenantErrors = make(map[ids.UUID]string)
	}
...
func (f *fakeTenantStore) DeleteTenantInstance(ctx context.Context, tenantID ids.UUID) error {
...
type fakeRuntimeClient struct {
	resultsByTenant map[ids.UUID]RuntimeApplyResult
	errByTenant     map[ids.UUID]error
}
